...
| `WalletButton` | `(props: WalletButtonProps) => JSX.Element` | `{ className?, style?, label?, color?, textColor?, onError?(e) }`. Self-contained connect button; once connected it renders its own `WalletDrawer`. Pass `color` to recolor just the connect button — `textColor` (icon + label) is auto-picked for contrast, or set it explicitly. While disconnected, also renders a small "Use an existing passkey" link that calls `reconnect()` — for restoring a wallet after storage loss (cleared site data, private browsing) without creating a new passkey. |
